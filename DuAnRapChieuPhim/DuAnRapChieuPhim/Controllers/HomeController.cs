...
namespace DuAnRapChieuPhim.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        [SavePreviousPage]
        public ActionResult LocHD(int? page, DateTime? ngayDat)
        {
            ...
            // Lọc theo ngày nếu ngày được chỉ định
            var veQuery = db.HoaDons.Where(u => u.MaKH == nguoidung.MaKH && u.TrangThai.Contains("Đã thanh toán"));
            if (ngayDat.HasValue)
            {
                veQuery = veQuery.Where(u => u.NgayDat.Value.Date == ngayDat.Value.Date);
            }

            // Sắp xếp kết quả theo ngày đặt giảm dần
            veQuery = veQuery.OrderByDescending(u => u.NgayDat);

            // Phân trang cho kết quả
            var ve = veQuery.ToPagedList(pageNumber, pageSize);
        ...
        public ActionResult ThanhToanQuaThe(int? diemThuong,string vc)
        {
            Session["diemthuong"] = diemThuong;
            var nguoidung = Session["KH"] as ThongTinCaNhan;

            List<int> listGhe = Session["ListGhe"] as List<int>;
            List<ComboDat> comboList = Session["CB"] as List<ComboDat>;
            decimal tienve = 0;
			...
			if (vc != "")
			{
				voucher = db.Vouchers.SingleOrDefault(u => u.MaVC == vc);
				Session["vc"] = vc;
				if (voucher.LoaiGiamGia.Contains("Phim"))
				{
					tienve = tienve - tienve * (decimal)(voucher.GiamGiaPhanTram / 100);
				}
				else
				{
					tiencb = tiencb - tiencb * (decimal)(voucher.GiamGiaPhanTram / 100);

				}
			}
			// Bây giờ yourList chứa thông tin của các combo và tongTien là tổng tiền cần thanh toán
			decimal tong = tienve + tiencb;
            if (nguoidung.Hang == "VIP")
            {
                tong = tong - (tong * 7 /100);
            }
            if (nguoidung.Hang == "VVIP")
            {
                tong = tong - (tong * 1 / 10);
            }
            if (diemThuong>0)
            {
                tong = tong - diemThuong.Value * 1000;
            }
			/*if (vc != "" )
            {
                var voucher = db.Vouchers.SingleOrDefault(u => u.MaVC == vc);
                tong = tong - (tong * (decimal)voucher.GiamGiaPhanTram / 100 );
                Session["vc"] = vc;
            }*/
        ...
        public ActionResult Combo(float totalGiaTien, string selectedSeats, int malc)
        {
            LichChieu lichChieu = db.LichChieus.SingleOrDefault(u => u.MaChieuPhim == malc);
            ViewBag.LichChieu = lichChieu;
            ViewBag.TienGhe = totalGiaTien;
            Session["TG"] = totalGiaTien;
            ...
            var nguoi = Session["KH"] as ThongTinCaNhan;
            var s = db.ThongTinCaNhans.SingleOrDefault(u => u.MaKH == nguoi.MaKH);
            ViewBag.khach = s.DiemThuong;
        ...
        public ActionResult TrangThanhToan(string selectedCombos)
        {
            if (!string.IsNullOrEmpty(selectedCombos))
            {
                ...
                foreach (var combo in comboList)
                {
                    var comboId = combo.MaCB;
                    var quantity = combo.SoLuong;
                    var cb = db.Combos.FirstOrDefault(c => c.MaCB == comboId);

                    if (cb != null)
                    {
                        // Nếu MaCB tồn tại, tính tổng tiền và cộng vào biến tổng
                        float giaTien = (float)cb.Gia.Value;
                        float thanhTien = giaTien * quantity;
                        totalComboPrice += thanhTien;

                    }
                }
                var khach = Session["KH"] as ThongTinCaNhan;
                ViewBag.TongCb = totalComboPrice;
                ViewBag.khach = khach.DiemThuong;
                ViewBag.TienGhe = Session["TG"];
                Session["CB"] = comboList;
            }
        ...
        {
            int diemThuong = (int)Session["diemthuong"];
            var nguoidung = Session["KH"] as ThongTinCaNhan;
            string vc = Session["vc"] as string;
            if (Request.QueryString.Count > 0)
            {
                string vnp_HashSecret = ConfigurationManager.AppSettings["vnp_HashSecret"]; //Chuoi bi mat
                var vnpayData = Request.QueryString;
                ...
                if (checkSignature)
                {
                    if (vnp_ResponseCode == "00" && vnp_TransactionStatus == "00")
                    {
                        ...
                        var dh = db.HoaDons.SingleOrDefault(u => u.MaHoaDon == madh);
                        dh.TrangThai = "Đã thanh toán";
                        var nd = db.ThongTinCaNhans.SingleOrDefault(u=> u.MaKH == nguoidung.MaKH);
                        nd.ThongTinChiTieu += dh.TongTien;
                        nd.DiemThuong -= diemThuong;
                        nd.DiemThuong = nd.DiemThuong + (int)(dh.TongTien / 10000); 
                        if (nd.ThongTinChiTieu >= 2000000)
                        {
                            nd.Hang = "VIP";
                        }
                        else if (nd.ThongTinChiTieu >= 10000000)
                        {
                        ...
                        if (vc != null)
                        {
							var v = db.Vouchers.SingleOrDefault(u => u.MaVC == vc);
        ...
        [HttpGet]
        public ActionResult CheckVoucher(string voucherCode)
        {
            // Thực hiện kiểm tra mã voucher và trả về thông tin voucher
            var voucher = db.Vouchers.SingleOrDefault(u => u.MaVC == voucherCode);

            if (voucher != null && voucher.NgayHetHan >= DateTime.Now && voucher.TrangThai != "Đã sử dụng")
            {

                return Json(new { isValid = true, discount = voucher.GiamGiaPhanTram, loai = voucher.LoaiGiamGia }, JsonRequestBehavior.AllowGet);
            }
            else
            {
        ...
        public ActionResult ChonGhe(int id,int MaChieuPhim)
        {
            ...
            var phim = db.Phims.SingleOrDefault(u=>u.MaPhim==lc.MaPhim);
            Session["Ten"] = phim.TenPhim;
            Session["Hinh"] = phim.AnhBia;
    ...
    .ToList();
        ...
        private void SendNewPasswordByEmail(string userEmail, string newPassword)
        {
            ...
            string smtpUsername = "cuongnuyenvan415@gmail.com";
            string smtpPassword = "tehhuipbeewkmqzw";

            // Địa chỉ email người gửi
            string fromEmail = "cuongnuyenvan415@gmail.com";
        ...
        private void SendEmail(long id)
        {
            ...
            string senderEmail = "cuongnuyenvan415@gmail.com";
            string senderPassword = "tehhuipbeewkmqzw";
            string smtpServer = "smtp.gmail.com";
        ...
        [SavePreviousPage]
        // GET: Home
        public ActionResult DoiVoucher()
        {
            var ng = Session["KH"] as ThongTinCaNhan;
            var nguoidung = db.ThongTinCaNhans.SingleOrDefault(u => u.MaKH == ng.MaKH);
            ViewBag.LoaiVoucher = db.LoaiVouchers.ToList();
            ViewBag.Diem = nguoidung.DiemThuong;
            return View();
        }
        [SavePreviousPage]
        public ActionResult VoucherToi(int? page)
        {
            var ng = Session["KH"] as ThongTinCaNhan;
            var nguoidung = db.ThongTinCaNhans.SingleOrDefault(u => u.MaKH == ng.MaKH);
            var vouchers = db.Vouchers.Where(u => u.MaKH == nguoidung.MaKH).ToList();
        ...
        [SavePreviousPage]
        public ActionResult TrangDoiVoucher(int maVoucher)
        {
            var ng = Session["KH"] as ThongTinCaNhan;
            var voucher = db.LoaiVouchers.SingleOrDefault(u => u.id == maVoucher);
            var nguoi = db.ThongTinCaNhans.SingleOrDefault(u => u.MaKH == ng.MaKH);
            nguoi.DiemThuong -= voucher.Diem;
            Voucher vc = new Voucher
            {
                img = voucher.img,
                NgayHetHan = DateTime.Today.AddDays(30),
                GiamGiaPhanTram = voucher.GiamGia,
                LoaiGiamGia = voucher.LoaiGiamGia,
                MaVC = GenerateVoucherCode(),
            ...
            voucher.sl -= 1;
            db.SubmitChanges();
            return RedirectToAction("DoiVoucher");
        }
        static string GenerateVoucherCode()
        {
            DateTime now = DateTime.Now;
            string formattedDate = now.ToString("yyyyMMddHHmmssfff");

            // Thêm ký tự ngẫu nhiên
            Random random = new Random();
            string randomChars = new string(Enumerable.Repeat("ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789", 5)
